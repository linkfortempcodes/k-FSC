function [L,output]=kFSC(X,d,k,lambda,options)
...
while i<=maxiter
    ...
    switch solver
        case 0
            C_new=update_C_Jacobi(X,D,C,d,k,lambda);
        case 1
            C_new=update_C_GaussSeidel(X,D,C,d,k,lambda);
        case 2
            [C_new,Q]=update_C_GaussSeidel_extrop(X,D,C,d,k,lambda,dfC,Q,i);
            dfC=C-C_new;
    end
...
%             Y(i,:)=sum(abs(C((i-1)*d+1:i*d,:)));
    ...
    case 're'
        ...
        for i=1:k
            ...
            E(i,:)=sum((X-Di*C_t).^2);
            %E(i,:)=sum((X-D(:,(i-1)*d+1:i*d)*C((i-1)*d+1:i*d,:)).^2);
        end
        [~,L]=min(E);
end
output.D=D;
